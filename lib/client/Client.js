...
const RESTHandler = require('../util/RESTHandler');

const MessageStore = require('../util/Stores/MessageStore');
...
class Client extends EventEmitter {
     ...
     * @param {Array} [options.intents=['GUILDS', 'GUILD_MESSAGES', 'GUILD_PRESENCES']] The list of intent groups you want to receive. See here for the intent group names and what events you receive: https://discordapp.com/developers/docs/topics/gateway#list-of-intents
     * @param {number} [options.messageCacheCheckTime=60] How long to wait in seconds between clearing the cache. Longer times mean the cache will be emptied less often, but messages will stay in the cache for longer.
     ...
     */
    constructor(token, options = defaultClientOptions) {
        super();
        ...
        if (options.cacheMessages) {
            this.messages = new MessageStore(this, null, options.messageCacheMaxAge, options.messageCacheMaxSize);
            this.messages.timer(options.messageCacheCheckTime);
        }
        else this.messages = new MessageStore(this, null, 0, 0);
