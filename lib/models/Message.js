...
class Message {
    ...
    constructor(client, obj, user = undefined, guild = undefined, channel = undefined) {
        ...
        this.embeds = obj.embeds; // TODO: --- MAJOR TODO NOW ---
        ...
        if (obj.member !== undefined) {
            this.member = new Member(client, obj.member, user, guild) || null;
            guild.members.set(user.id, this.member);
        }

        if (obj.reactions !== undefined) {
     ...
     */
    async react(emoji) {
        const parsedEmoji = encodeURIComponent(emoji);
