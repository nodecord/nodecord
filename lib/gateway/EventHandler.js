...
class EventHandler {
    ...
    async handle(t, d) {
        ...
        if (t === events.MESSAGE_CREATE[0]) {
            ...
            if (this.client.options.cacheMessages) this.client.messages.add(d.id, message);
            
            this.client.emit(events.MESSAGE_CREATE[1], message);
        }

        /* MESSAGE_UPDATE */
        if (t === events.MESSAGE_UPDATE[0]) {
            const guild = this.client.guilds.get(d.guild_id);
            const author = new User(this.client, d.author);
            const channel = guild.channels.get(d.channel_id);

            let oldMessage = null;
            let newMessage = new Message(this.client, d, author, guild, channel);

            if (this.client.options.cacheMessages) {
                oldMessage = this.client.messages.get(d.id) || null;

                this.client.messages.delete(d.id);
                this.client.messages.add(d.id, newMessage);
            }

            this.client.emit(events.MESSAGE_UPDATE[1], oldMessage, newMessage);
        }

        /* MESSAGE_DELETE */
        if (t === events.MESSAGE_DELETE[0]) {
            let message = null;

            if (this.client.options.cacheMessages) {
                message = this.client.messages.get(d.id) || null;

                this.client.messages.delete(d.id);
            }

            this.client.emit(events.MESSAGE_DELETE[1], message);
        }
        ...
        if (t === events.GUILD_UPDATE[0]) {
            const oldGuild = this.client.guilds.get(d.id);
            const newGuild = new Guild(this.client, d);

            this.client.guilds.delete(d.id);
            this.client.guilds.set(d.id, newGuild);

            return this.client.emit(events.GUILD_UPDATE[1], oldGuild, newGuild);
        }
