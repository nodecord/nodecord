...
const { baseURL, wsCloseCodes, opcodes } = require('../util/Constants');
...
class Gateway {
    ...
    async handle(resume = false) {
        ...
        this.socket.on('close', (code, reason = null) => {
            ...
            this.client.emit('debug', `Gateway closed with code ${code}`);
            ...
            if (wsCloseCodes[code]) {
                ...
                if (code === 4000 || code === 4007 || code === 4009) {
                   this.client.emit('error', `Gateway closed with code ${code}: ${closeObj.message}`);
            ...
            if (op === opcodes.INVALID_SESSION) {
                ...
                setTimeout(() => {
                    if (d === true && this.client.ws.sessionID) {
                        // TODO: try to resume
                        throw new Error(`Resuming on invalid session is not supported yet`);
                    } else {
